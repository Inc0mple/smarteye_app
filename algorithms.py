...
def extract_keypoints_parallel(queue, args, self_counter, other_counter, consecutive_frames, event):
    try:
        cam, tagged_df = cv2.VideoCapture(0), None
...
def alg2_sequential(queue, argss, consecutive_frames, event):
    ...
    window_names = [argss[0].video if isinstance(args.video, str) else 'Cam '+str(args.video) for args in argss]
    [cv2.namedWindow(window_name) for window_name in window_names]
    while True:
        # if not queue1.empty() and not queue2.empty():
        if not queue.empty():
            dict_frames = queue.get()
            ...
            num_matched, new_num, indxs_unmatched = match_ip(ip_sets[0], kp_frames, lstm_sets[0], num_matched, max_length_mat)
            ...
            img, output_videos[0] = show_tracked_img(dict_frames, ip_sets[0], num_matched, output_videos[0], argss[0])
            # print(img1.shape)
